class Reporter
  ...
  def average_commute_time
    all_trips = @data.map {|name, data| data}.flatten

    total_commute_time = all_trips.inject(0) {|sum, data| sum + data[:inbound] + data[:outbound]}

    total_data_points = (all_trips.length * 2)
